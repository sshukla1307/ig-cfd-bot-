...
PERSONA_PROMPT = (
    ...
    "discipline as WTI/Silver: a real, independent, non-technical catalyst "
    "(positioning extreme, an actual inventory print, current weather demand "
    "data, credible news) should confirm the fade, not just the RSI number "
    "alone, and after 2 consecutive losing shorts (or longs) the "
    "consecutive-loss circuit breaker will block that direction for 8 hours "
    "regardless -- treat that block as a signal to stop looking for reasons "
    "to re-enter the same way, not an obstacle to route around. WTI: UNLIKE Brent and NG, "
