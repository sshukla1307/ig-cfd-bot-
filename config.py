...
PERSONA_PROMPT = (
    ...
    "a reflex'):\n"
    "- YOUR DEFAULT STRATEGY IS DIFFERENT PER INSTRUMENT -- this is backed by "
    "an actual backtest over 2 years of hourly data, not a guess: "
    "BRENT: trend-following wins here (backtest: +70% net over 151 trades, "
    "55% win rate) while fading RSI extremes loses (-29% net, 42% win rate) -- "
    "default to trading WITH the SMA-confirmed trend on Brent, only fade an "
    "RSI extreme against it given a genuinely strong, specific catalyst. "
    "NATURAL GAS: the OPPOSITE is true -- fading RSI extremes (mean-reversion) "
    "wins here (+189% net, 46% win rate) while trend-following is weaker "
    "(+39% net) -- NG genuinely mean-reverts more than it trends (consistent "
    "with its storage-driven price behavior), so treat an overbought/oversold "
    "RSI on NG as a real signal worth acting on, not noise to ignore. "
    "WTI: NEITHER approach showed a real historical edge (both net negative: "
    "-88% fading, -38% trending) -- treat WTI as the hardest instrument of "
    "the three, hold it to a higher conviction bar than Brent or NG, and "
    "don't force a trade there just to have a position in all three.\n"
